		...
		<view class="main-content">
			<view class="nannyDetails-top">
				...
				<view class="nannyDetails-nav-fabulous-nav">
					<u-icon name="heart-fill" color="#FFBB9F" size="28rpx"></u-icon>
			...
			<view class="nannyDetails-nav">
				...
				<view class="income" @click="gotobill">
					<image class="income-src" src="../../static/main/income.png" mode=""></image>
					<view class="numbox">
...
<script>
	...
	export default {
		...
		methods: {
			...
			callStationmaster() {
				uni.makePhoneCall({
					phoneNumber: this.mineData.station_phone,
					success: (res) => {},
					fail: (res) => {}
				});
			},
			...
			getbill() {
				...
				bill({
					...
					if (res.code == 0) {
						this.lastmonth_shouru = res.data.list.lastmonth_shouru
						this.shouru = res.data.list.shouru
					} else {
						this.$refs.uToast.show({
							title: res.msg
						})
