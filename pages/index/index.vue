			...
			<view class="Receivingorders-nav-list" v-if="acceptList.length > 0">
				<view class="Receivingorders-nav-list-item" v-for="(item,index) in acceptList" :key="index">
					<view class="Receivingorders-nav-list-item-title">
						<view class="Receivingorders-nav-list-item-title-left">
							<view class="Receivingorders-nav-list-item-title-left-type">
								<image class="ontrial" src="../../static/index/ontrial.png" mode=""></image>
							</view>
					...
					<view class="switchBut">
						<button type="info" v-for="(item) in openList" class="butStyle" @click="open(item.text)">
							{{item.text}}
...
<script>
	...
	//正式环境
	// var wsUrl = "wss://socket.top0530.top";
	...
	export default {
		...
		methods: {
				...
				getAcceptList({
					...
					if (res.code == 0) {
						...
						arr.map((item, index) => {
							...
							item.distance = "1.2km";
							let addressArr = item.address.split("省");
							item.address = addressArr[1]
							if (item.remark) {
								item.list = [{
										"name": "面积",
			...
			open(text) {
				if (text == "查看完整信息") {
					this.openList[0].text = "收起"
					this.openList[0].icon = "arrow-up"
					this.news = true
				}
				if (text == "收起") {
					this.openList[0].text = "查看完整信息"
					this.openList[0].icon = "arrow-down"
					this.news = false
				}
			},
			//确认是否不接这个单的弹窗
			...
			doUpData() {
				...
				uni.downloadFile({ //执行下载
					url: 'https://static.top0530.top/appversion/nanny/jsa_nanny_1.0.apk', //下载地址
