		...
		<view class="app-content">
			<view class="container">
				<view class="step-wrap step-block">
					<view class="step-title">上传个人资料(1/3)</view>
				</view>
				<view class="step-personal step-block">
					<view class="personal-title">上传身份证（正反面）</view>
					<view class="personal-subtitle">请上传身份证完善身份信息</view>
					<view class="personal-form">
						<view class="personal-upload flexbox justify">
							...
							<view class="personal-upload-item flexbox center middle">
								<image class="upload-bg" :src="form.idcard_image_back == '' ? '../../static/personal/idcard-bg2.png' : form.idcard_image_back"></image>
								<view class="upload-postion">
									<image class="upload-icon" src="../../static/personal/icon-camera.png" mode="widthFix" @click="uploadImg"
									 data-type="2"></image>
								</view>
		...
		methods: {
							...
							uni.uploadFile({
								...
								success: (res) => {
									...
									if (type == 1) {
										...
										this.form.idcard_image_back = imageData.data.url;
									}
								}
							});
						}
			...
			submit() {
				this.$refs.uForm.validate(valid => {
					if (valid) {
						uni.setStorageSync('form1', JSON.stringify(this.form))
						uni.navigateTo({
							url: './step2'
						})
					} else {
